...
```{r message=FALSE}
pal <- colorFactor(palette = topo.colors(5), domain = macleish_trails$name)
...
difficulty_map <- leaflet() %>%
  addTiles() %>%
  addPolylines(data = hiking_trails, color = ~pal(name))%>%
  addMarkers(data = st_cast(interior_trails, "POINT"), popup = ~name)

#second one in pal is Western & entry trail
#third one is Eastern & Vernal 
difficulty_map
```
